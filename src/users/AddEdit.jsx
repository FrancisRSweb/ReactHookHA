...
function AddEdit({ history, match }) {
    ...
    const validationSchema = Yup.object().shape({
        ...
        password: Yup.string()
            ...
            .min(6, 'La contraseña requiere minimo 6 carateres')
            .max(12,'La contraseña tiene un maximo de 12 caracteres'),
        confirmPassword: Yup.string()
            .transform(x => x === '' ? undefined : x)
            .when('password', (password, schema) => {
                if (password || isAddMode) return schema.required('Las contraseñas no coinciden');
            })
    ...
    function createUser(data) {
        return userService.create(data)
            .then(() => {
                alertService.success('Usuario añadido', { keepAfterRouteChange: true });
                history.push('.');
            })
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} onReset={reset}>
            ...
            <div className="form-row">
                ...
                <div className="form-group col">
                    <label>Fecha</label>
                    <input name="role" type="date" ref={register} className={`form-control ${errors.role ? 'is-invalid' : ''}`} />
